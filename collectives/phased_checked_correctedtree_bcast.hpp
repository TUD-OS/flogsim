...
class PhasedCheckedCorrectedTreeBroadcast : public Collective
{
  ...
  static Time tree_phase_end(int k)
  {
  ...
  typedef PhasedCheckedCorrectedTreeBroadcast coll_t;
  ...
  public:
    void post_next_message(coll_t coll, TaskQueue &tq)
    {
      ...
      if (tq.now() < tree_phase_end(coll.k)) {
        return;
      }
...
public:
  ...
  virtual void accept(const InitTask &task, TaskQueue &tq)
  {
    ...
    tq.schedule(TimerTask::make_new(tree_phase_end(k), root.id));
  }
};
